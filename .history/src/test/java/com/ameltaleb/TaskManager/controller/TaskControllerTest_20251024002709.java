...
public class TaskControllerTest {
    ...
    void testUpdateTask_404() throws Throwable {
        ...
        //given
        when(taskService.updateTask(eq(NON_EXISTING_TASK_ID), any(Task.class))).thenReturn(taskToUpdate);

        //when and then

        ((MockHttpServletRequestBuilder) mockMvc.perform(put("/api/tasks/{id}", NON_EXISTING_TASK_ID)))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(taskToUpdate))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.error").value("Not Found"));
